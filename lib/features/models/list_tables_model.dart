...
class ListTablesModel {
  ...
  factory ListTablesModel.fromMap(Map<String, dynamic> map) {
    return ListTablesModel(
      meals: List<TablesModel>.from(
        (map['meals'] as List<int>).map<TablesModel>(
          (x) => TablesModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
      justifications: List<TablesModel>.from(
        (map['justifications'] as List<int>).map<TablesModel>(
          (x) => TablesModel.fromMap(x as Map<String, dynamic>),
        ),
      ),
    );
  ...
  factory ListTablesModel.fromJson(String source) =>
      ListTablesModel.fromMap(json.decode(source) as Map<String, dynamic>);
}
